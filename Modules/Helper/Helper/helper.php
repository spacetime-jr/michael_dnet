...
/**
 * @param $file
 * @param null $type, possibility: product-feat, product-gallery, user-profpic
 * @return \Illuminate\Contracts\Routing\UrlGenerator|string
 */
function storeImage($file, $type = null, $ref_id = null)
{

    try {
        $media = new Media;
        $media->name = pathinfo($file->getClientOriginalName(), PATHINFO_FILENAME);
        $media->type = \File::mimeType($file);
        $media->size = \File::size($file);
        $media->format = \File::extension($file->getClientOriginalName());
        ...
        switch($type){
            case 'product-feat':
                $media->guid = 'products/feat_image/' . $ref_id . '/' . $media->slug . '.' . $media->format;

                $media->save();

                $img = \Image::make($file);

                $path = public_path('products/feat_image/' . $ref_id . '/');
            ...
            case 'product-gallery':
                $media->guid = 'products/gallery/' . $ref_id . '/' . $media->slug . '.' . $media->format;

                $media->save();

                $img = \Image::make($file);

                $path = public_path('products/gallery/' . $ref_id . '/');
