...
class EmployeeController extends Controller
{
    ...
    public function create()
    {
        ...
        if (\Sentinel::getUser()->hasAccess('employee.create')) {
            $_SESSION['menu'] = 'employee';
    ...
    public function store(Request $request)
    {
        if (\Sentinel::getUser()->hasAccess('user.create')) {
            ...
            return redirect()->route('employee.edit', $user->id)->with('success', 'User saved');
        } else {
            return redirect()->route('dashboard')->with('error', 'Access dennied!');
        }
    }
    ...
    public function edit($id)
    {
        ...
        if (\Sentinel::getUser()->hasAccess('employee.edit')   || \Auth::user()->id == $id) {
            try{
                $_SESSION['menu'] = 'employee';
                ...
                $roles = $roles->where('id', '<>', env('SUPERADMIN_ID','1'));
                $roles = $roles->get();

                $currentRole = \Sentinel::findById($user->id)->roles[0]->slug;
                return view('users::employee.edit', [
                    'user' => $user,
                    'roles' => $roles,
    ...
    public function update($id, Request $request)
    {
        if (\Sentinel::getUser()->hasAccess('employee.edit')  || \Auth::user()->id == $id) {
            ...
            return redirect()->route('employee.edit', $id)->with('success', 'User updated');
        } else {
            return redirect()->route('dashboard')->with('error', 'Access dennied!');
        }
    }
    ...
    {
        if (\Sentinel::getUser()->hasAccess('employee.delete')) {
            $_SESSION['menu'] = 'employee';
            $user = User::find($id);
//            $user->delete();
            $user->email = null;
    ...
    public function ajaxListEmployee(Request $request)
    {
        if (\Sentinel::getUser()->hasAccess('employee.list')) {
            $employee_role_id = env('EMPLOYEE_ROLE_ID',3);
            $columns = array(
                0 => 'username',
                1 => 'email',
                2 => 'fullname',
...
//            5 => 'updated_at',
            ...
            // COunt total Employee user
            $role = \Sentinel::findRoleById($employee_role_id); // Employee role ID
            ...
            $users = \Sentinel::findRoleById($employee_role_id)->users();
            ...
            if (!empty($users)) {
                foreach ($users as $user) {
                    ...
                    //if (\Sentinel::getUser()->hasAccess('employee.show'))
                        //$show = "<a href='" . route('employee.show', $user->id) . "' title='Show' ><span class='icon-file-text'></span></a>";

                    if (\Sentinel::getUser()->hasAccess('employee.edit'))
                        $edit = "<a href='" . route('employee.edit', $user->id) . "' title='Edit' ><span class='icon-pencil'></span></a>";

                    if (\Sentinel::getUser()->hasAccess('employee.delete'))
                        $delete = "<a href='" . route('employee.deleteUser', $user->id) . "' title='Delete' class='confirmAction'><span class='icon-trash'></span></a>";
